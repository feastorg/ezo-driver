// This code is for the Atlas Scientific wifi hydroponics kit with dissolved oxygen that uses the Adafruit ESP32-S3 TFT Feather as its computer.
...
void step4()
{
  ...
  {                                                                    // if the DO reading was successful (back in step 1)
  ...
  canvas.fillRect(0, 99, 240, 33, 0xfe80);
  canvas.setTextColor(ST77XX_BLACK);
  canvas.print("DO");
  canvas.setCursor(70, 123);
  if (DO.get_error() == Ezo_board::SUCCESS)
  { // if the EC reading was successful (back in step 1)
