...
void receive_name(Ezo_board &Device)
{
  ...
  {
    if (length > 1)
    { // only overwrite if we have a name
      memset(Device.get_name(), 0, 17);
...
bool scan_devices()
{
  ...
  {                                         // step through the i2C addresses
    ...
    if (error == 0)
    {                                            // if its possible to communicate with it
...
void setup()
{
  ...
  if (!scan_devices())
  {
    Serial.println("No EZO devices found!");
  }
  receive_devicetypes();
  query_name();
  Seq.reset();
