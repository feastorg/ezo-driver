...
{
  if (Serial.available())
  { // if theres any characters in the UART buffer
...
void process_command(const String &string_buffer, Ezo_board *device_list[], uint8_t device_list_len, Ezo_board *&default_board)
{
  ...
  {                                                                       // if the command starts with ALL:
    ...
    for (uint8_t i = 0; i < device_list_len; i++)
    { // go through our list of boards and get their response
  ...
  {                                             // if we received any other commands
    ...
    { // if it contains a colon character
      ...
      { // if its valid
        ...
        for (uint8_t i = 0; i < device_list_len; i++)
        {
          if ((name_to_find == device_list[i]->get_name()) || (address == device_list[i]->get_address()))
          {                                 // if the address matches one of the boards in the list
        ...
        if (addr_found)
        { // then send the rest of the command to that board
          default_board->send_cmd(string_buffer.substring(index + 1).c_str());
        }
        else
        { // otherwise print that we didnt find
    ...
    { // if theres no colon just pass the command to the default board
...
void list_devices(Ezo_board device_list[], uint8_t device_list_len, Ezo_board *default_board)
{
  ...
  for (uint8_t i = 0; i < device_list_len; i++)
  { // go thorugh the list of boards
    if (default_board == &device_list[i])
    {                       // if its our default board
