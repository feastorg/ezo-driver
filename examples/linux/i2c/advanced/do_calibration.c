...
Defaults: /dev/i2c-1, address 97, step status, and operating recalibration temperature 25.0 C.
Assumptions: the connected device is a D.O. circuit in I2C mode and the probe is in the matching zero or atmospheric calibration condition.
...
static ezo_result_t preview_readings(ezo_i2c_device_t *device,
  ...
  for (index = 0; index < preview_samples; ++index) {
    ...
    result = ezo_do_read_response_i2c(device, output_mask, &reading);
    if (result != EZO_OK) {
      return result;
    }
...
int main(int argc, char **argv) {
  ...
  if (!ezo_example_parse_i2c_options(argc, argv, 97U, &options, &next_arg)) {
    fprintf(stderr,
            "usage: %s [device_path] [address] [--step=status|low|high|clear|operating-temp-recal] "
  ...
  if (result == EZO_OK) {
    result = ezo_do_send_temperature_query_i2c(&session.device, &hint);
  }
  if (result == EZO_OK) {
    ezo_example_wait_hint(&hint);
    result = ezo_do_read_temperature_i2c(&session.device, &temperature);
  }
  if (result == EZO_OK) {
    result = ezo_do_send_salinity_query_i2c(&session.device, &hint);
  }
  if (result == EZO_OK) {
    ezo_example_wait_hint(&hint);
    result = ezo_do_read_salinity_i2c(&session.device, &salinity);
  }
  if (result == EZO_OK) {
    result = ezo_do_send_pressure_query_i2c(&session.device, &hint);
  }
  if (result == EZO_OK) {
    ezo_example_wait_hint(&hint);
    result = ezo_do_read_pressure_i2c(&session.device, &pressure);
  }
  if (result == EZO_OK) {
    result = ezo_do_send_calibration_query_i2c(&session.device, &hint);
  }
  if (result == EZO_OK) {
    ezo_example_wait_hint(&hint);
    result = ezo_do_read_calibration_status_i2c(&session.device, &calibration);
  }
  if (result != EZO_OK) {
  ...
  if (apply_requested) {
    if (step == STEP_OPERATING_TEMP_RECAL) {
      result = ezo_do_send_temperature_set_i2c(&session.device, operating_temperature_c, 2, &hint);
      if (result == EZO_OK) {
        ezo_example_wait_hint(&hint);
      }
    }
    if (result == EZO_OK && step == STEP_CLEAR) {
      result = ezo_do_send_clear_calibration_i2c(&session.device, &hint);
    } else if (result == EZO_OK && step != STEP_STATUS) {
      result = ezo_do_send_calibration_i2c(&session.device, calibration_point_for_step(step), &hint);
    }
    if (result != EZO_OK) {
      ezo_example_close_i2c(&session);
