...
int main(int argc, char **argv) {
  ...
  if (result == EZO_OK) {
    result = ezo_rtd_send_logger_query_i2c(&session.device, &hint);
  }
  if (result == EZO_OK) {
    ezo_example_wait_hint(&hint);
  ...
  if (apply_requested) {
    result = ezo_rtd_send_scale_set_i2c(&session.device, planned_scale, &hint);
    if (result == EZO_OK) {
      ezo_example_wait_hint(&hint);
      result = ezo_rtd_send_logger_set_i2c(&session.device, planned_logger_interval, &hint);
    }
    if (result == EZO_OK) {
      ezo_example_wait_hint(&hint);
    }
