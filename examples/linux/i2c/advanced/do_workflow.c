...
int main(int argc, char **argv) {
  ...
  if (result == EZO_OK) {
    result = ezo_do_send_read_with_temp_comp_i2c(&session.device,
  ...
  if (result == EZO_OK) {
    ezo_example_wait_hint(&hint);
    result = ezo_do_read_response_i2c(&session.device,
  ...
  if (apply_requested) {
    result = ezo_do_send_temperature_set_i2c(&session.device, planned_temperature_c, 2, &hint);
    if (result == EZO_OK) {
      ezo_example_wait_hint(&hint);
      result = ezo_do_send_salinity_set_i2c(&session.device,
                                            planned_salinity_value,
    ...
    if (result == EZO_OK) {
      ezo_example_wait_hint(&hint);
      result = ezo_do_send_pressure_set_i2c(&session.device, planned_pressure_kpa, 3, &hint);
    }
    if (result == EZO_OK) {
      ezo_example_wait_hint(&hint);
      result = ezo_do_send_output_set_i2c(&session.device,
