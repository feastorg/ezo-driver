...
Defaults: /dev/i2c-1, address 111, step status, reference 25.0.
Assumptions: the connected device is a HUM circuit in I2C mode and the air-temperature reference is stable.
...
static ezo_result_t preview_readings(ezo_i2c_device_t *device,
  ...
  for (index = 0; index < preview_samples; ++index) {
    ...
    result = ezo_hum_read_response_i2c(device, enabled_mask, &reading);
    if (result != EZO_OK) {
      return result;
    }
...
int main(int argc, char **argv) {
  ...
  if (apply_requested) {
    if (step == STEP_CLEAR) {
      result = ezo_hum_send_clear_temperature_calibration_i2c(&session.device, &hint);
