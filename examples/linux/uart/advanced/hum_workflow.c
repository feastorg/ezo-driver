...
Assumptions: the connected device is a HUM circuit and response-code mode can be bootstrapped.
...
int main(int argc, char **argv) {
  ...
  if (!ezo_example_parse_uart_options(argc,
                                      ...
                                      &next_arg)) {
    fprintf(stderr,
            "usage: %s [device_path] [baud] [--reference-temp=25.0] "
  ...
  if (apply_requested) {
    result = ezo_hum_send_output_set_uart(&session.device,
