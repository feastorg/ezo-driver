...
Defaults: /dev/ttyUSB0 at 9600 baud, step status, midpoint reference 7.00.
Assumptions: the connected device is a pH circuit, response-code mode can be bootstrapped, and the probe is in the matching calibration solution.
...
int main(int argc, char **argv) {
  ...
  if (!ezo_example_parse_uart_options(argc,
                                      ...
                                      &next_arg)) {
    fprintf(stderr,
            "usage: %s [device_path] [baud] [--step=status|mid|low|high|clear] "
  ...
  if (apply_requested) {
    if (step == STEP_CLEAR) {
      result = ezo_ph_send_clear_calibration_uart(&session.device, &hint);
    } else if (step != STEP_STATUS) {
      result = ezo_ph_send_calibration_uart(&session.device,
