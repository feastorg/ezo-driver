...
Defaults: /dev/ttyUSB0 at 9600 baud, step status, reference 225 mV.
Assumptions: the connected device is an ORP circuit, response-code mode can be bootstrapped, and the probe is in a stable reference solution.
...
static ezo_result_t preview_readings(ezo_uart_device_t *device,
  ...
  for (index = 0; index < preview_samples; ++index) {
    result = ezo_orp_send_read_uart(device, &hint);
    if (result != EZO_OK) {
      return result;
    }
    ezo_example_wait_hint(&hint);
    result = ezo_orp_read_response_uart(device, &reading);
    if (result != EZO_OK) {
      return result;
    }
...
int main(int argc, char **argv) {
  ...
  if (apply_requested) {
    if (step == STEP_CLEAR) {
      result = ezo_orp_send_clear_calibration_uart(&session.device, &hint);
    } else if (step == STEP_CALIBRATE) {
      result = ezo_orp_send_calibration_uart(&session.device, reference_mv, 0, &hint);
    }
    if (result != EZO_OK) {
      ezo_example_close_uart(&session);
